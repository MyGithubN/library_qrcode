...
public class CodeCreator {
	...
	private BufferedImage createCodeImage(CodeModel info) {
		...
		try {
			BitMatrix bm = writer.encode(contents,
			...
			info.setBottomStart(new int[] { locationTopLeft[0],
			              locationBottomRight[1] });
			...
			for (int x = 0; x < w; x++) {
				for (int y = 0; y < h; y++) {
					img.setRGB(x, y, bm.get(x, y) ? BLACK
					              : WHITE);
				}
			}
	...
	public void createCodeImage(CodeModel info, OutputStream output) {
		...
		if (logoFile != null && logoFile.exists()) {
			try {
				...
				if (ratio > 0) {
					logoWidth = logoWidth > width * ratio ? (int) (width * ratio)
					              : logoWidth;
					logoHeight = logoHeight > height * ratio ? (int) (height * ratio)
					              : logoHeight;
				}
				int x = (width - logoWidth) / 2;
				int y = (height - logoHeight) / 2;
				Graphics g = bm.getGraphics();
				g.drawImage(logoImg, x, y, logoWidth, logoHeight,
				              null);
				String desc = info.getDesc();
				// int whiteWidth = 8;
				if (!StringUtils.isEmpty(desc)) {
					int whiteWidth = info.getHeight()
					              - info.getBottomEnd()[1];
					// width =
					// info.getBottomEnd()[0]-info.getBottomStart()[0];
					// height = info.getBottomEnd()[1]+1;
					Font font = new Font("黑体", Font.BOLD,
					              info.getFontSize());
					int fontHeight = g.getFontMetrics(font)
					              .getHeight();
					// 计算需要多少行
					int lineNum = 1;
					...
					int wordTopMargin = 4;
					BufferedImage bm1 = new BufferedImage(width,
					              height + totalFontHeight
					                            + wordTopMargin
					                            - whiteWidth,
					              BufferedImage.TYPE_INT_RGB);
					Graphics g1 = bm1.getGraphics();
					if (totalFontHeight + wordTopMargin
					              - whiteWidth > 0) {
					...
					width = info.getBottomEnd()[0]
					              - info.getBottomStart()[0];
					height = info.getBottomEnd()[1] + 1;
					currentLineLen = 0;
		...
		try {
			ImageIO.write(bm,
			              StringUtils.isEmpty(info.getFormat()) ? info
			                            .getFormat() : info
			                            .getFormat(), output);
		} catch (Exception e) {
			e.printStackTrace();
