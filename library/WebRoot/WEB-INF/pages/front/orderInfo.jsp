	        ...
	        <tr>
	        	<td>
		        	<s:if test="#request.bookStudent.yn == 0">
							...
							<s:elseif test="#request.bookStudent.status == 1">
								<form action="<%=path%>/order_s_qrcode?oid=<s:property value="#request.bookStudent.id" />" method="get">
				        			<input type="submit" class="btn btn-lg btn-primary" id="login_submit" value=" 确定借阅"/>
				        			<input type="hidden" name="oid" value="<s:property value="#request.bookStudent.id" />"/>
		        				</form>
		        				<br/>
		        				<form action="<%=path%>/order_delete?oid=<s:property value="#request.bookStudent.id" />" method="get">
				        			<input type="submit" class="btn btn-lg btn-primary" id="login_submit" value=" 取消借阅"/>
				        			<input type="hidden" name="oid" value="<s:property value="#request.bookStudent.id" />"/>
		        				</form>
							</s:elseif>
	        	</td>
